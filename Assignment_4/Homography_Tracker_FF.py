...
class Homography_Tracker_FF:
    ...
    def calc_stitched_scene_homography(self, good_init_prev, good_init_next, prev_img, good_prev, next_img, good_next):
        ...
        status, stitched = cv2.Stitcher.create(cv2.Stitcher_SCANS).stitch([warped_img_next, warped_img_prev])
    ...
    def concate_images(self, BigImage, newimage, width , height , center):
        x1 = int(center[0] - (width/2))
        x2 = int(center[0] + (width/2))
        y1 = int(center[1] - (height/2))
        y2 = int(center[1] + (height/2))
        BigImage[x1:x2, y1:y2,:]  = cv2.resize(newimage,(height, width),interpolation=cv2.INTER_CUBIC)
        return BigImage
    ...
    def test(self):
        ...
        mosaic = np.zeros((700,1400, 3) , dtype=np.uint8)
        center = [350 , 700]
        ...
        while cap.isOpened():
            ...
            if len(next_pts[status == 1]) >= MIN_MATCH_COUNT:
                H, _ = cv2.findHomography(next_pts[status == 1], init_pts[status == 1], cv2.RANSAC, 5.0)
                warped_img_prev = cv2.warpPerspective(next_img, H, flags=cv2.WARP_INVERSE_MAP, dsize=(next_img.shape[1], next_img.shape[0]))

                s, stitched = cv2.Stitcher.create(cv2.Stitcher_SCANS).stitch([warped_img_prev, init_img])

                # if s == cv2.Stitcher_OK:
                #     s, stitched = cv2.Stitcher.create(cv2.Stitcher_SCANS).stitch([init_img, stitched])

                if s == cv2.Stitcher_OK:
                    # stitched = cv2.resize(stitched, (init_img.shape[1], init_img.shape[0]), interpolation=cv2.INTER_CUBIC)
                    # stitched = self.cut_stitched(stitched)



                    w,h,_ = stitched.shape

                    show_stitched = cv2.resize(stitched, (int(h / 2), int(w / 2)), interpolation=cv2.INTER_CUBIC)
                    mosaic = self.concate_images(mosaic, show_stitched, int(w / 2), int(h / 2), center)
                    cv2.imshow("total", mosaic)
                    # cv2.imshow("total", show_stitched)
                    print(stitched.shape)
                else:
                    # cv2.imshow("init_img", cv2.resize(prev_stitched, (512, 512), interpolation=cv2.INTER_CUBIC))
                    # cv2.imshow("next_img", cv2.resize(warped_img_prev, (512, 512), interpolation=cv2.INTER_CUBIC))
                    print("Error!")
                ...
                print("Not enough matches are found - %d/%d" % (len(next_pts[status == 1]), MIN_MATCH_COUNT))
    ...
    def test2(self):
        print("Running homography tracking process..")
        cap, init_img = self.get_video_capturer()
        prev_points = init_pts = self.fetch_key_points(init_img)  # self.get_points_from_user()
        previmage = stitched = init_img.copy()

        mosaic = np.zeros((700, 1400, 3), dtype=np.uint8)
        center = [350, 700]
        # pictures = []
        # pictures.append(init_img)
        i = 0
        while cap.isOpened():
            ...
            if not ret:
                ...
                next_pts, next_status = self.calc_next_point(previmage, next_img, prev_points)
                prev_points = next_pts
                previmage = next_img

                if len(next_pts[next_status == 1]) >= MIN_MATCH_COUNT:
                    # Handling of the correspondences
                    # Computing the Homography & Stitiching process
                    # Computing Homography transformation with the initial point's frame
                    H, _ = cv2.findHomography(next_pts[next_status == 1], init_pts[next_status == 1], cv2.RANSAC, 5.0)

                    # Warping the next frame with the Homography matrix
                    warped = cv2.warpPerspective(next_img, H, flags=cv2.WARP_INVERSE_MAP, dsize=(next_img.shape[1], next_img.shape[0]))

                    # pictures.append(warped)
                    # print(len(pictures))

                    status, stitched_check = cv2.Stitcher.create(cv2.Stitcher_SCANS).stitch([warped, init_img])
                    # status, stitched_check = cv2.Stitcher.create(cv2.Stitcher_SCANS).stitch(pictures)
                    if stitched_check is not None:
                        print("----------")
                        print(warped.shape)
                        print(stitched.shape)
                        print(len(next_pts[next_status == 1]))
                        stitched = stitched_check
                    else:
                        print(len(next_pts[next_status == 1]))
                        print(warped)
                        print("Error!")
                    ...
                    print("Not enough matches are found - %d/%d" % (len(next_pts[next_status == 1]), MIN_MATCH_COUNT))
